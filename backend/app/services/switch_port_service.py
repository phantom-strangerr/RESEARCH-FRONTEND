...
def isolate_port(db: Session, port_id: UUID, isolation: SwitchPortIsolate, original_vlan: int = None):
    db_port = get_port_by_id(db, port_id)
    if not db_port:
        return None
    if original_vlan is not None:
        db_port.original_vlan = original_vlan
...
def seed_ports(db: Session):
    sample_ports = [
        ...
        {"port_number": 4, "status": "isolated", "device_ip": "192.168.1.107", "device_mac": "DE:AD:BE:EF:CA:FE", "device_name": "IoT-Sensor-04", "vlan": 30, "speed": "100M", "bytes_sent": 3.2, "bytes_received": 2.1, "errors": 45, "drops": 23, "isolation_reason": "Replay Attack Detected", "isolated_by": "edge_ml"},
        {"port_number": 5, "status": "isolated", "device_ip": "192.168.1.108", "device_mac": "FF:EE:DD:CC:BB:AA", "device_name": "Camera-05", "vlan": 30, "speed": "100M", "bytes_sent": 10.3, "bytes_received": 8.5, "errors": 67, "drops": 34, "isolation_reason": "Botnet Activity detected", "isolated_by": "edge_ml"},
    ...
    for p in sample_ports:
        if get_port_by_number(db, p["port_number"]):
            continue
        isolated_at = datetime.now(timezone.utc) if p["status"] == "isolated" else None
        db_port = SwitchPort(
            port_number=p["port_number"], status=p["status"], device_ip=p.get("device_ip"),
            device_mac=p.get("device_mac"), device_name=p.get("device_name"), vlan=p.get("vlan"),
            speed=p.get("speed"), bytes_sent=p.get("bytes_sent", 0), bytes_received=p.get("bytes_received", 0),
            errors=p.get("errors", 0), drops=p.get("drops", 0), isolation_reason=p.get("isolation_reason"),
            isolated_at=isolated_at, isolated_by=p.get("isolated_by"),
        )
        db.add(db_port)
